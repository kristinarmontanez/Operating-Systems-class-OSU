...
use std::thread; // Shown in materials.
...
let mut each_thread : Vec<std::thread::JoinHandle<usize>> = Vec::new();
for element in equal_part 
    {each_thread.push( thread::spawn(move || map_data( &element)));}

    
// 4. Collect intermediate sums from all threads.
let mut intermediate_sums_2 = Vec::new();
for sum in each_thread 
    {intermediate_sums_2.push( sum.join().unwrap());} 
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>
    ...
    // Per Ed Discussion: 
    // https://edstem.org/us/courses/14269/discussion/907706
    // First, divide the total number by number of groups and 
    // round down. Then distribute the remaining indexes starting
    // with the first group.
    // Adjust our first round of looping through vectors, otherwise
    // we only end up with 4 partitions in example run "5 153".
    let first_round = num_partitions + 1;
    // This sub_vector will allow us to push each sub_vector to final_vector
    // Once each time sub_vector is done being populated.
    let mut sub_vector : Vec<usize> = Vec::new();
    // Run through the first full set of elements that divide
    // perfectly into the partitions.
    for elements in 0..first_round 
        // If we have not ran out of elements before remainder
        // elements, add the elements to the vector.
        {if elements >= 1 
            // If we have elements left, place them 
            // in the final_vector.
            {final_vector.push(sub_vector.clone());
            sub_vector.clear()}
        // If we have started over, add our elements 
        // to the sub_vector. 
        if elements < num_partitions 
            {for _part in 0..partition_size 
                {let num_elements = v.len();
                if index < num_elements 
                    {let sub_index = v[index];
                    sub_vector.push(sub_index);
                    // Don't forget to move up each
                    // time for elements.
                    index += 1;}}}}
    // Now we can distribute the rest of the 
    // remainder elements. 
    if remainder >= 1
        {for remain in 0..remainder
            {let left_over_index = v[index];
            final_vector[remain].push(left_over_index);
            // Again, move up each time for remaining
            // elements.
            index += 1;}}
    // Return final Vector.
    final_vector}
