...
Date:           Nov 29, 2021
Class:          CS344
Project:        Assignment 5 - dec_server.c
...
*/
char decryptMessage(char key, char buffer) 
    ...
    // original ASCII range from the uppercase letters and space.
    buffer_ASCII = (buffer_ASCII - key_ASCII) % 27;
    if (buffer_ASCII < 0) buffer_ASCII = buffer_ASCII + 27;
                    ...
                    if (strcmp(client_id, buffer) == 0) 
                        ...
                        while (!line) 
                            ...
                            for (size_t l = 0; l < strlen(buffer); l++) 
                                {current = decryptMessage(key[l], buffer[l]);
